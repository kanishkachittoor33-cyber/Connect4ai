...
class BamlAsyncClient:
    ...
    async def GetAIMove(self, gameState: types.GameState,
        baml_options: BamlCallOptions = {},
    ) -> types.AIMove:
        # Check if on_tick is provided
        if 'on_tick' in baml_options:
            # Use streaming internally when on_tick is provided
            stream = self.stream.GetAIMove(gameState=gameState,
                baml_options=baml_options)
            return await stream.get_final_response()
        else:
            # Original non-streaming code
            result = await self.__options.merge_options(baml_options).call_function_async(function_name="GetAIMove", args={
                "gameState": gameState,
            })
            return typing.cast(types.AIMove, result.cast_to(types, types, stream_types, False, __runtime__))
...
class BamlStreamClient:
    ...
    def GetAIMove(self, gameState: types.GameState,
        baml_options: BamlCallOptions = {},
    ) -> baml_py.BamlStream[stream_types.AIMove, types.AIMove]:
        ctx, result = self.__options.merge_options(baml_options).create_async_stream(function_name="GetAIMove", args={
            "gameState": gameState,
        })
        return baml_py.BamlStream[stream_types.AIMove, types.AIMove](
          result,
          lambda x: typing.cast(stream_types.AIMove, x.cast_to(types, types, stream_types, True, __runtime__)),
          lambda x: typing.cast(types.AIMove, x.cast_to(types, types, stream_types, False, __runtime__)),
          ctx,
        )
...
class BamlHttpRequestClient:
    ...
    async def GetAIMove(self, gameState: types.GameState,
        ...
        result = await self.__options.merge_options(baml_options).create_http_request_async(function_name="GetAIMove", args={
            "gameState": gameState,
        }, mode="request")
        return result
